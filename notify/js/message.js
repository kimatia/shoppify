;(function () {
    ...
    var api = {
         ...
         */
        bellDraw: function () {
            var bellHtml = "<span data-type='1' class='message-bell-btn' "+ (option.msgUnReadData > 0?("title='"+ option.msgUnReadData + " New Message(s)'"):"") +">" +
                "<i class='fa fa-bell-o '></i>"+ (option.msgUnReadData > 0?"<span class='badge-dot'></span>":"") +"</span>" +
                "<span data-type='2' class='message-bell-btn' "+ (option.noticeUnReadData > 0?("title='"+ option.noticeUnReadData +" New Message(s)'"):"") +">" +
        ...
        contentListDraw: function(type) {
            ...
            var div = "<div class='message-frame-header'><span style='height: 40px;line-height: 40px;text-align: center;width: 50px;    display: inline-block;'>"+ (type==1?"Message":"Notify") +"</span><button class='message-btn message-btn-blue message-btn-header' data-type='"+ type +"'>Mark As Read</button></div>"+
            ...
            if (type == 1) {
                if (option.msgData && option.msgData.length > 0) {
                    ...
                    div += "<div class='message-none-msg'>No Messages</div>";
                }
                div += "</div>";

                if (this.nodeList.length < option.msgData.length) {
                    div += "<div class='message-footer' data-type='"+ type +"'><span>Load More</span></div>";
                }
            } else if (type == 2) {
